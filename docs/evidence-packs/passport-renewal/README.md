...
- **Goal ID:** `renew_ordinary_egyptian_passport`
- **Goal (Arabic):** تجديد جواز سفر مصري عادي
- **Goal (English):** Renew an ordinary Egyptian passport
- **Procedure ID:** `ordinary_domestic_passport_renewal`
- **Procedure (Arabic):** استخراج جواز سفر بدل جواز سفر منتهي أو ممتلئ الصفحات داخل مصر
- **Procedure (English):** Obtain a replacement passport for an expired or page-full ordinary passport inside Egypt
...
The current Public Services Guide exposes “replacement for an expired or page-full passport” as a distinct service. Lost- and damaged-passport replacements are separate services and remain outside this Procedure.

## 2. Supported boundary

The evidence pack is intended to support the ordinary path only when all of the following are true:
...
| `existing_passport_state` | enum | `expired`, `pages_full`, `valid_with_pages`, `lost`, `damaged`, `none` | distinguish renewal from adjacent Passport Procedures |
| `passport_class` | enum | `ordinary`, `other` | exclude non-ordinary passports |
...
Questions about whether the person physically possesses each required document are optional readiness questions, not procedure-selection questions. They should not be asked until the applicable requirement is known.
...
The stable Document Type identifies the item only. Quantities, applicability, classification, and evidence belong to the Procedure Version claim.
...
- basis: project safety requirement from #1; not a government claim
...
Detailed source records and exact-passage fragments are in `sources.md`. The consequential findings are:
...
- [x] Concrete Goal and Procedure identity recorded.
- [x] Ordinary domestic boundary and recognized unsupported cases recorded.
...
- [x] Claim-level IDs, classifications and evidence-link IDs recorded.
...
- [x] Stale/conflicting evidence preserved rather than silently normalized.
