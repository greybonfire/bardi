...
class ProcedureVersionFreshnessTests(unittest.TestCase):
    ...
    def test_military_version_boundary_is_inclusive_and_rule_snapshot_is_selected(self) -> None:
        ...
        missing_claim = next(
            item
            for item in after.plan.historical_claims
            if item.id == "mil.basis.missing_war_or_terror_relative"
        )
        self.assertEqual(
            tuple(source.source_id for source in missing_claim.sources),
            (
    ...
    def test_future_published_versions_are_upcoming_but_drafts_are_not(self) -> None:
        ...
        self.assertEqual(tuple(item.procedure_version_id for item in result.plan.upcoming_versions), ("passport.future",))
    ...
    def test_no_current_version_reports_upcoming_without_leaking_draft(self) -> None:
        ...
        draft = replace(
            future,
            procedure=replace(future.procedure, version_id="passport.draft.only", publication_state="draft"),
            service_point_associations=tuple(
        ...
        self.assertEqual(tuple(item.procedure_version_id for item in result.upcoming_versions), ("passport.future.only",))

    def test_overlapping_published_intervals_are_invalid(self) -> None:
        passport = self.catalog.fixtures["ordinary_domestic_passport_renewal"]
        second = replace(
            passport,
            procedure=replace(passport.procedure, version_id="passport.overlap", effective_from=date(2026, 1, 1)),
            service_point_associations=tuple(
        ...
        self.assertTrue(any(code.startswith("overlapping_published_procedure_versions:") for code in validate_catalog(catalog)))
        result = run_scenario(
            knowledge=catalog,
            goal_id="get_egyptian_passport",
            facts=self.passport_facts(),
    ...
    def test_stale_consequential_claim_is_local_and_keeps_reliable_claims(self) -> None:
        ...
        self.assertIn(stale.id, {item.id for item in result.plan.historical_claims})
        self.assertIn("passport.requirement.photos", {item.id for item in result.plan.checklist})
        self.assertTrue(result.plan.historical_claims[0].current_value_unknown)

    def test_discrepancy_is_internal_and_changes_only_affected_claim_trust(self) -> None:
        passport = self.catalog.fixtures["ordinary_domestic_passport_renewal"]
        ...
        variant = replace(
            ...
            discrepancies=(
                EvidenceDiscrepancy(
                    ...
                    status="open",
                    rationale="internal editorial conflict",
                ),
            ),
        )
        result = run_scenario(
        ...
        self.assertNotIn("internal editorial conflict", repr(result))
    ...
    def test_historical_claim_projection_has_arabic_english_identity_parity(self) -> None:
        ...
        self.assertEqual(en.plan.historical_claims[0].sources, ar.plan.historical_claims[0].sources)
        self.assertNotEqual(en.plan.historical_claims[0].text, ar.plan.historical_claims[0].text)
